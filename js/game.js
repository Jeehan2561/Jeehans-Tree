...
function softcap(value, cap, power = D(0.1)) {
...
VERSION.withoutName = modInfo.name + " v" + VERSION.num + (VERSION.pre ? " Pre-Release " + VERSION.pre : VERSION.pre ? " Beta " + VERSION.beta : "")
...
function gameLoop(diff) {
	...
	for (let x = 0; x <= maxRow; x++){
		for (item in TREE_LAYERS[x]) {
			...
			player[layer].resetTime = Decimal.add(new Decimal (player[layer].resetTime), new Decimal (diff))
